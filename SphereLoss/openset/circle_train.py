...
import losses_nets
from losses_nets import CircleLoss, convert_label_to_similarity
...
net = losses_nets.ResNet50Embedder(feature_dim=256, dropout_p=0.2, pretrained=True).to(device)
net.feature = True

criterion = CircleLoss(m=args.m, gamma=args.gamma).to(device)

if args.optimizer.lower() == 'sgd':
    optimizer = optim.SGD(net.parameters(), lr=args.lr, momentum=0.9, weight_decay=.0001)
elif args.optimizer.lower() == 'adam':
    optimizer = optim.Adam(net.parameters(), lr=args.lr, weight_decay=.0001)
else:
    optimizer = optim.AdamW(net.parameters(), lr=args.lr, weight_decay=.0001)
...
best_eer, no_improve, patience = 1.0, 0, 10
...
for epoch in range(args.epochs):
    ...
    for imgs, lbls in train_loader:
        ...
        sp, sn = convert_label_to_similarity(embeds, lbls)
    ...
    with torch.no_grad():
        for imgs, lbls in val_loader:
            emb_val.append(net(imgs.to(device)).cpu())
            lbl_val.append(lbls)
